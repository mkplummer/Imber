...
class Target:
    ...
    def observed_properties(self,spectra_file,apparent_magnitude,magnitude_band,zero,file_type = 'FITS'):
...
class Analytical_Model:
    ...
    def rotational_plus_instrument_kernel(self):
        ...
        LPrb = np.copy(self.Zrb)
        " The std dev is c/R"
        sig = c/self.spectral_resolution
...
class Simulated_Observation:
    ...
    def __init__(self,instrument,model,texp,nexp):
    ...
    def compute_snr_spectral(self):
        ...
        # fig,ax = plt.subplots()
        # plt.plot(self.instrument_wave,instrument_flux,label = 'Instrument')
        # plt.plot(photometric_wave,photometric_flux,label = 'Photometric Band')
        # plt.legend()
        ...
        self.snr = (self.nexp**0.5)*Nobj/((Nobj+Nsky+Npix*(self.instrument.readout_noise**2)+Npix*self.instrument.dark_current*self.texp)**0.5)
        # fig, ax = plt.subplots()
    ...
    def compute_snr_photo(self):
        ...
        'S/N ratio'
        self.snr = (self.nexp**0.5)*Nobj/((Nobj+Nsky+self.instrument.Npix\
...
class Spectroscopic_Instrument:
    ...
    def initial_calculations(self,target):
        ...
        if self.spectral_band == 'MODHIS':
            ...
            sky_emission = np.interp(self.instrument_wave,self.wave,flux)
...
class NestedSampling():
    def __init__(self,target,time_samples,spectral_observation = 0,photo_observation = 0,\
                 free_contrast = False,free_radius = False,sampling_mode='dynamic',\
                         truths = 0,evolution_spot_number = False,evolution_rotation = False,\
                             num_bands = 0, num_spots = 0,amp1_mode = False,free_bias = False,\
                                 free_period = False):
        self.target = target
        ...
        self.amp1_mode = amp1_mode
        self.free_bias = free_bias
    ...
    def run(self):
            ...
            if self.evolution_boolean == True:
                ...
                if self.free_radius == True:
                    ...
                    if self.free_contrast == True:
                        ...
                        ndim = ndim
            if self.num_bands > 0:
                ndim = ndim+2+(3*self.num_bands)
            elif self.free_bias == True:
                ndim = ndim+1
        elif self.num_bands > 0:
                ndim = 2+(3*self.num_bands)
        if self.free_period == True:
                ndim = ndim+1
        ...
        def loglike(x):
            ...
            if data_mode == 'Spectroscopic':
                ...
                if numspot > 0:
                    ...
                    if self.free_bias == True:
                        model_full_time_lightcurve = (photo_model.light_curve)+bias-1
                    else:
                        model_full_time_lightcurve = photo_model.light_curve
                    if self.num_bands > 0:
                        model_full_time_lightcurve = model_full_time_lightcurve-1
                        tphoto = photo_model.time_hours
                        if self.amp1_mode == True:
                            if self.num_bands == 1:
                                phase = b1*np.pi/180
                                wave1 = a1*np.sin(2*np.pi*tphoto/P1+phase)
                                wave_curve = photo_mean+(photo_slope*tphoto)+wave1
                            elif self.num_bands == 2:
                                phase1,phase2 = b1*np.pi/180,b2*np.pi/180
                                wave1 = a1*np.sin(2*np.pi*tphoto/P1+phase1)
                                wave2 = a2*np.sin(2*np.pi*tphoto/P2+phase2)
                                wave_curve = photo_mean+(photo_slope*tphoto)+wave1+wave2
                        elif self.num_bands == 1:
                            wave1 = a1*np.sin(2*np.pi*tphoto/P1)+b1*np.cos(2*np.pi*tphoto/P1)
                            wave_curve = photo_mean+(photo_slope*tphoto)+wave1
                        elif self.num_bands == 2:
                            wave1 = a1*np.sin(2*np.pi*tphoto/P1)+b1*np.cos(2*np.pi*tphoto/P1)
                            wave2 = a2*np.sin(2*np.pi*tphoto/P2)+b2*np.cos(2*np.pi*tphoto/P2)
                            wave_curve = photo_mean+(photo_slope*tphoto)+wave1+wave2
                        elif self.num_bands == 3:
                            wave1 = a1*np.sin(2*np.pi*tphoto/P1)+b1*np.cos(2*np.pi*tphoto/P1)
                            wave2 = a2*np.sin(2*np.pi*tphoto/P2)+b2*np.cos(2*np.pi*tphoto/P2)
                            wave3 = a3*np.sin(2*np.pi*tphoto/P3)+b3*np.cos(2*np.pi*tphoto/P3)
                            wave_curve = photo_mean+(photo_slope*tphoto)+wave1+wave2+wave3
                        elif self.num_bands == 4:
                            wave1 = a1*np.sin(2*np.pi*tphoto/P1)+b1*np.cos(2*np.pi*tphoto/P1)
                            wave2 = a2*np.sin(2*np.pi*tphoto/P2)+b2*np.cos(2*np.pi*tphoto/P2)
                            wave3 = a3*np.sin(2*np.pi*tphoto/P3)+b3*np.cos(2*np.pi*tphoto/P3)
                            wave4 = a4*np.sin(2*np.pi*tphoto/P4)+b4*np.cos(2*np.pi*tphoto/P4)
                            wave_curve = photo_mean+(photo_slope*tphoto)+wave1+wave2+wave3+wave4
                        model_full_time_lightcurve = model_full_time_lightcurve+wave_curve
                elif self.num_bands > 0:
                    tphoto = photo_model.time_hours
                    if self.amp1_mode == True:
                        if self.num_bands == 1:
                            ...
                            phase1,phase2,phase3 = b1*np.pi/180,b2*np.pi/180,b3*np.pi/180
                            wave1 = a1*np.sin(2*np.pi*tphoto/P1+phase1)
                            wave2 = a2*np.sin(2*np.pi*tphoto/P2+phase2)
                            wave3 = a3*np.sin(2*np.pi*tphoto/P3+phase3)
                            wave_curve = photo_mean+(photo_slope*tphoto)+wave1+wave2+wave3
                        elif self.num_bands == 4:
                            phase1,phase2,phase3,phase4 = b1*np.pi/180,b2*np.pi/180,b3*np.pi/180,b4*np.pi/180
                            wave1 = a1*np.sin(2*np.pi*tphoto/P1+phase1)
                            wave2 = a2*np.sin(2*np.pi*tphoto/P2+phase2)
                            wave3 = a3*np.sin(2*np.pi*tphoto/P3+phase3)
                            wave4 = a4*np.sin(2*np.pi*tphoto/P4+phase4)
                            wave_curve = photo_mean+(photo_slope*tphoto)+wave1+wave2+wave3+wave4
                    elif self.num_bands == 1:
                        wave1 = a1*np.sin(2*np.pi*tphoto/P1)+b1*np.cos(2*np.pi*tphoto/P1)
                        wave_curve = photo_mean+(photo_slope*tphoto)+wave1
                        ...
                        wave4 = a4*np.sin(2*np.pi*tphoto/P4)+b4*np.cos(2*np.pi*tphoto/P4)
                        wave_curve = photo_mean+(photo_slope*tphoto)+wave1+wave2+wave3+wave4
                    model_full_time_lightcurve = wave_curve
                model_light_curve = np.interp(self.photo_observation.time_hours,photo_model.time_hours,model_full_time_lightcurve)
                # fig,ax = plt.subplots()
                # plt.scatter(self.photo_observation.time_hours,self.photo_observation.light_curve)
                # plt.scatter(self.photo_observation.time_hours,model_light_curve)
                diff = self.photo_observation.light_curve-model_light_curve
                ...
                photo_model = Analytical_Model(Star2,length_of_observation,self.time_samples,n,eps,spectral_resolution)
                photo_model.create_lightcurve()
                interp_light_curve = np.interp(self.photo_observation.time_hours,self.photo_model.time_hours,photo_model.light_curve)
                diff2 = self.photo_model.light_curve-interp_light_curve
        ...
        def ptform(u):
                ...
                if self.free_contrast == True:
                    if self.free_radius == True:
                        if self.evolution_boolean == True:
                            ...
                            x[1] = 180 + 180* (2. * u[1] - 1.) #Longitude
                            x[2] = 30. + 30 * (2. * u[2] - 1.) #Radius
                            x[3] = 30. + 30 * (2. * u[3] - 1.) #Radius Evolved
                            x[4] = 0.5 + 0.5 * (2. * u[4] - 1.) #Contrast
                            x[5] = 0.5 + 0.5 * (2. * u[5] - 1.) #Contrast Evolved
                            # if self.num_bands == 1:
                            #     x[6] = 0.0 + 5.0 * (2. * u[6] - 1.) # a1
                            #     x[7] = 0.0 + 5.0 * (2. * u[7] - 1.) # b1
                            #     x[8] = 50.0 + 50.0 * (2. * u[8] - 1.) #Period 1 
                            #     x[9] = 1.0 + 0.25 * (2. * u[9] - 1.) # photo mean
                ...
                if self.free_contrast == True:
                    if self.free_radius == True:
                        if self.evolution_boolean == True:
                            ...
                            x[1] = 180 + 180 * (2. * u[1] - 1.) #Longitude
                            x[2] = 30. + 30 * (2. * u[2] - 1.) #Radius
                            x[3] = 30. + 30 * (2. * u[3] - 1.) #Radius
                            x[4] = 0.5 + 0.5 * (2. * u[4] - 1.) #Contrast
                            x[5] = 0.5 + 0.5 * (2. * u[5] - 1.) #Contrast Evolved
                            x[6] = 90 * (2. * u[6] - 1.) #Latitude
                            x[7] = 180 + 180 * (2. * u[7] - 1.) #Longitude
                            x[8] = 30. + 30 * (2. * u[8] - 1.) #Radius 
                            x[9] = 30. + 30 * (2. * u[9] - 1.) #Radius Evolved
                            x[10] = -0.5 + 0.5 * (2. * u[10] - 1.) #Contrast
                            x[11] = -0.5 + 0.5 * (2. * u[11] - 1.) #Contrast Evolved
                        else:
                            x[0] = 90. * (2. * u[0] - 1.) #Latitude
                            x[1] = 180 + 180 * (2. * u[1] - 1.) #Longitude
                            x[2] = 30. + 30 * (2. * u[2] - 1.) #Radius 
                            ...
                            x[5] = 180 + 180 * (2. * u[5] - 1.) #Longitude
                            x[6] = 30. + 30 * (2. * u[6] - 1.) #Radius 
            ...
            if self.amp1_mode == True:
                if self.num_bands == 1:
                    x[0] = 2.5 + 2.5 * (2. * u[0] - 1.) # a1
                    x[1] = 180 + 180 * (2. * u[1] - 1.) # Phase
                    x[2] = 2.5 + 2.5 * (2. * u[2] - 1.) #Period 1 
                    x[3] = 0.0 + 10.0 * (2. * u[3] - 1.) # photo slope
                    x[4] = 100.0 + 25.0 * (2. * u[4] - 1.) # photo mean
                elif self.num_bands == 2:
                    x[0] = 2.5 + 2.5 * (2. * u[0] - 1.) # a1
                    x[1] = 180 + 180 * (2. * u[1] - 1.) # Phase 1
                    x[2] = 2.5 + 0.5  * (2. * u[2] - 1.) #Period 1
                    x[3] = 2.5 + 2.5 * (2. * u[3] - 1.) # a2
                    # x[5] = ((x[2]+2.0)/2) + ((x[2]-2.0)/2) * (2. * u[5] - 1.)
                    x[5] = 1.5 + 0.5 * (2. * u[5] - 1.)
                    x[4] = 180 + 180 * (2. * u[4] - 1.) # Phase 2
                    x[6] = 0.0 + 10.0 * (2. * u[6] - 1.) # photo slope
                    x[7] = 100.0 + 25.0 * (2. * u[7] - 1.) # photo mean
                    
                    
                    # 
                    # x[7] = 100.0 + 25.0 * (2. * u[7] - 1.) # photo mean
                    # x[6] = 0.0 + 10.0 * (2. * u[6] - 1.) # photo slope
                    # x[5] = 2.5 + 1.0 * (2. * u[5] - 1.) #Period 1 
                    # x[4] = 180 + 180 *  (2. * u[4] - 1.) # Phase 1
                    # x[3] = 2.5 + 2.5 * (2. * u[3] - 1.) # a1
                    # x[2] = 1.25 + 0.5 * (2. * u[2] - 1.)
                    # x[1] = 180 + 180 *  (2. * u[1] - 1.) # Phase
                    # x[0] = 2.5 + 2.5 * (2. * u[0] - 1.) # a2
                elif self.num_bands == 3:
                    x[0] = 2.5 + 2.5 * (2. * u[0] - 1.) # a1
                    x[1] = 180 + 180 * (2. * u[1] - 1.) # Phase 1
                    x[2] = 2.41 + 0.5  * u[2] #Period 1
                    x[3] = 2.5 + 2.5 * (2. * u[3] - 1.) # a2
                    x[4] = 180 + 180 * (2. * u[4] - 1.) # Phase 2
                    x[5] =  2.0 + 0.41 * u[5] # Period 3
                    # x[5] = 2.5 + 0.5 * (2. * u[5] - 1.) # Period 2
                    # x[5] = ((x[2]+2.0)/2) + ((x[2]-2.0)/2) * (2. * u[5] - 1.)
                    x[6] = 2.5 + 2.5 * (2. * u[6] - 1.) # a3
                    x[7] = 180 + 180 * (2. * u[7] - 1.) # Phase 3
                    # x[8] = ((x[5]+1.0)/2) + ((x[5]-1.0)/2) * (2. * u[8] - 1.)
                    x[8] = 1.5 + 0.5 * (2. * u[8] - 1.) # Period 3
                    # x[8] = 2.0 + 0.41 * u[8] # Period 3
                    x[9] = 0.0 + 10.0 * (2. * u[9] - 1.) # photo slope
                    x[10] = 100.0 + 25.0 * (2. * u[10] - 1.) # photo mean
                elif self.num_bands == 4:
                    x[0] = 2.5 + 2.5 * (2. * u[0] - 1.) # a1
                    x[1] = 180 + 180 * (2. * u[1] - 1.) # Phase 1
                    x[2] = 2.5 + 1.0  * (2. * u[2] - 1.) #Period 1
                    x[3] = 2.5 + 2.5 * (2. * u[3] - 1.) # a2
                    x[4] = 180 + 180 * (2. * u[4] - 1.) # Phase 2
                    # x[5] = 1.5 + 0.5 * (2. * u[5] - 1.) # Period 2
                    x[5] = ((x[2]+2.0)/2) + ((x[2]-2.0)/2) * (2. * u[5] - 1.)
                    x[6] = 2.5 + 2.5 * (2. * u[6] - 1.) # a3
                    x[7] = 180 + 180 * (2. * u[7] - 1.) # Phase 3
                    x[8] = ((x[5]+1.0)/2) + ((x[5]-1.0)/2) * (2. * u[8] - 1.)
                    # x[8] = 0.75 + 0.25 * (2. * u[8] - 1.) # Period 3
                    x[9] = 2.5 + 2.5 * (2. * u[9] - 1.) # a4
                    x[10] = 180 + 180 * (2. * u[10] - 1.) # Phase 3
                    x[11] = ((x[8]+0.5)/2) + ((x[8]-0.5)/2) * (2. * u[11] - 1.)
                    # x[8] = 0.75 + 0.25 * (2. * u[8] - 1.) # Period 3
                    x[12] = 0.0 + 10.0 * (2. * u[12] - 1.) # photo slope
                    x[13] = 100.0 + 25.0 * (2. * u[13] - 1.) # photo mean



            elif self.num_bands > 0:
                if numspot > 0:
                    x[-1] = 1.0 + 0.25 * (2. * u[-1] - 1.) # photo mean
                    x[-2] = 0.0 + 0.1 * (2. * u[-2] - 1.) # photo slope
                    if self.num_bands == 1:
                        x[-3] = 4.0 + 4.0 * (2. * u[-2] - 1.) #Period 1 
                        x[-4] = 0.0 + 0.1 * (2. * u[-3] - 1.) # b1
                        x[-5] = 0.0 + 0.1 * (2. * u[-4] - 1.) # a1
                    elif self.num_bands == 2:
                        x[-3] = 4.0 + 4.0 * (2. * u[-3] - 1.) #Period 1 
                        x[-4] = 0.0 + 0.1 * (2. * u[-4] - 1.) # b1
                        x[-5] = 0.0 + 0.1 * (2. * u[-5] - 1.) # a1
                        x[-6] = 4.0 + 4.0 * (2. * u[-6] - 1.) #Period 2 
                        x[-7] = 0.0 + 0.1 * (2. * u[-7] - 1.) # b2
                        x[-8] = 0.0 + 0.1 * (2. * u[-8] - 1.) # a2
                    elif self.num_bands == 3:
                        x[-3] = 4.0 + 4.0 * (2. * u[-3] - 1.) #Period 1 
                        x[-4] = 0.0 + 0.1 * (2. * u[-4] - 1.) # b1
                        x[-5] = 0.0 + 0.1 * (2. * u[-5] - 1.) # a1
                        ...
                        x[-7] = 0.0 + 0.1 * (2. * u[-7] - 1.) # b2
                        x[-8] = 0.0 + 0.1 * (2. * u[-8] - 1.) # a2
                        x[-9] = 4.0 + 4.0 * (2. * u[-9] - 1.) #Period 3 
                        x[-10] = 0.0 + 0.1 * (2. * u[-10] - 1.) # b3
                        x[-11] = 0.0 + 0.1 * (2. * u[-11] - 1.) # a3
                else:
                    if self.num_bands == 1:
                        # x[0] = 0.0 + 5.0 * (2. * u[0] - 1.) # a1
                        # x[1] = 0.0 + 5.0 * (2. * u[1] - 1.) # b1
                        # x[2] = 2.5 + 2.5 * (2. * u[2] - 1.) #Period 1 
                        # x[3] = 0.0 + 10.0 * (2. * u[3] - 1.) # photo slope
                        # x[4] = 100.0 + 25.0 * (2. * u[4] - 1.) # photo mean
                        x[0] = 0.0 + 0.05 * (2. * u[0] - 1.) # a1
                        x[1] = 0.0 + 0.05 * (2. * u[1] - 1.) # b1
                        x[2] = 2.5 + 2.5 * (2. * u[2] - 1.) #Period 1 
                        x[3] = 0.0 + 0.1 * (2. * u[3] - 1.) # photo slope
                        x[4] = 1.0 + 0.25 * (2. * u[4] - 1.) # photo mean
                    elif self.num_bands == 2:
                        # x[7] = 1.0 + 0.25 * (2. * u[7] - 1.) # photo mean
                        # x[6] = 0.0 + 0.1 * (2. * u[6] - 1.) # photo slope
                        # x[5] = 2.5 + 1.0 * (2. * u[5] - 1.) #Period 1 
                        # x[4] = 0.0 + 0.05 * (2. * u[4] - 1.) # b1
                        # x[3] = 0.0 + 0.05 * (2. * u[3] - 1.) # a1
                        # x[2] = ((x[5]+1.5)/2) + ((x[5]-1.5)/2) * (2. * u[2] - 1.)
                        # # x[2] = 2.5 + 1.0 * (2. * u[2] - 1.) #Period 2 
                        # x[1] = 0.0 + 0.05 * (2. * u[1] - 1.) # b2
                        # x[0] = 0.0 + 0.05 * (2. * u[0] - 1.) # a2
                        x[7] = 100.0 + 25.0 * (2. * u[7] - 1.) # photo mean
                        x[6] = 0.0 + 10.0 * (2. * u[6] - 1.) # photo slope
                        x[5] = 2.5 + 1.0 * (2. * u[5] - 1.) #Period 1 
                        x[4] = 0.0 + 5.0 * (2. * u[4] - 1.) # b1
                        x[3] = 0.0 + 5.0 * (2. * u[3] - 1.) # a1
                        # x[2] = 2.5 + 1.0 * (2. * u[2] - 1.) #Period 2
                        x[2] = ((x[5]+1.0)/2) + ((x[5]-1.0)/2) * (2. * u[2] - 1.) #Period 3
                        x[1] = 0.0 + 5.0 * (2. * u[1] - 1.) # b2
                        x[0] = 0.0 + 5.0 * (2. * u[0] - 1.) # a2
                    elif self.num_bands == 3:
                        x[10] = 100.0 + 25.0 * (2. * u[10] - 1.) # photo mean
                        x[9] = 0.0 + 10.0 * (2. * u[9] - 1.) # photo slope
                        x[8] = 2.5 + 1.0 * (2. * u[8] - 1.) #Period 1 
                        x[7] = 0.0 + 2.5 * (2. * u[7] - 1.) # b1
                        x[6] = 0.0 + 2.5 * (2. * u[6] - 1.) # a1
                        x[5] = ((x[8]+1.5)/2) + ((x[8]-1.5)/2) * (2. * u[5] - 1.)
                        # x[5] = 2.5 + 1.0 * (2. * u[5] - 1.) #Period 2 
                        # x[5] = 1.25 + 0.5 * (2. * u[5] - 1.) #Period 2 
                        x[4] = 0.0 + 2.5 * (2. * u[4] - 1.) # b2
                        x[3] = 0.0 + 2.5 * (2. * u[3] - 1.) # a2
                        x[2] = ((x[5]+1.0)/2) + ((x[5]-1.0)/2) * (2. * u[2] - 1.) #Period 3
                        # x[2] = 2.5 + 2.5 * (2. * u[2] - 1.) #Period 3
                        x[1] = 0.0 + 2.5 * (2. * u[1] - 1.) # b3
                        x[0] = 0.0 + 2.5 * (2. * u[0] - 1.) # a3
                        # x[10] = 1.0 + 0.25 * (2. * u[10] - 1.) # photo mean
                        # x[9] = 0.0 + 0.1 * (2. * u[9] - 1.) # photo slope
                        # x[8] = 2.5 + 1.0 * (2. * u[8] - 1.) #Period 1 
                        # # x[8] = 0.75 + 0.25 * (2. * u[8] - 1.) #Period 1 
                        # x[7] = 0.0 + 0.05 * (2. * u[7] - 1.) # b1
                        # x[6] = 0.0 + 0.05 * (2. * u[6] - 1.) # a1
                        # x[5] = ((x[8]+1.5)/2) + ((x[8]-1.5)/2) * (2. * u[5] - 1.) #Period 3
                        # # x[5] = 1.25 + 0.5 * (2. * u[5] - 1.) #Period 2 
                        # x[4] = 0.0 + 0.05 * (2. * u[4] - 1.) # b2
                        # x[3] = 0.0 + 0.05 * (2. * u[3] - 1.) # a2
                        # x[2] = ((x[5]+1.0)/2) + ((x[5]-1.0)/2) * (2. * u[2] - 1.) #Period 3
                        # # x[2] = 2.5 + 1.0 * (2. * u[2] - 1.) #Period 3
                        # x[1] = 0.0 + 0.05 * (2. * u[1] - 1.) # b3
                        # x[0] = 0.0 + 0.05 * (2. * u[0] - 1.) # a3
                    elif self.num_bands == 4:
                        x[13] = 100.0 + 25.0 * (2. * u[13] - 1.) # photo mean
                        x[12] = 0.0 + 10.0 * (2. * u[12] - 1.) # photo slope
                        x[11] = 3.5 + 2.0 * (2. * u[11] - 1.) #Period 1 
                        x[10] = 0.0 + 5.0 * (2. * u[10] - 1.) # b1
                        x[9] = 0.0 + 5.0 * (2. * u[9] - 1.) # a1 
                        x[8] = 3.5 + 2.0 * (2. * u[8] - 1.) #Period 1 
                        x[7] = 0.0 + 5.0 * (2. * u[7] - 1.) # b1
                        x[6] = 0.0 + 5.0 * (2. * u[6] - 1.) # a1
                        x[5] = 1.25 + 0.5 * (2. * u[5] - 1.) #Period 2 
                        x[4] = 0.0 + 2.0 * (2. * u[4] - 1.) # b2
                        x[3] = 0.0 + 2.0 * (2. * u[3] - 1.) # a2
                        x[2] = 0.55 + 0.55 * (2. * u[2] - 1.) #Period 3
                        x[1] = 0.0 + 2.0 * (2. * u[1] - 1.) # b3
                        x[0] = 0.0 + 2.0 * (2. * u[0] - 1.) # a3
                        # x[13] = 1.0 + 0.25 * (2. * u[13] - 1.) # photo mean
                        # x[12] = 0.0 + 0.1 * (2. * u[12] - 1.) # photo slope
                        # x[11] = 4.0 + 4.0 * (2. * u[11] - 1.) #Period 1 
                        # x[10] = 0.0 + 0.1 * (2. * u[10] - 1.) # b1
                        # x[9] = 0.0 + 0.1 * (2. * u[9] - 1.) # a1 
                        # x[8] = 4.0 + 4.0 * (2. * u[8] - 1.) #Period 1 
                        # x[7] = 0.0 + 0.1 * (2. * u[7] - 1.) # b1
                        # x[6] = 0.0 + 0.1 * (2. * u[6] - 1.) # a1
                        # x[5] = 4.0 + 4.0 * (2. * u[5] - 1.) #Period 2 
                        # x[4] = 0.0 + 0.1 * (2. * u[4] - 1.) # b2
                        # x[3] = 0.0 + 0.1 * (2. * u[3] - 1.) # a2
                        # x[2] = 4.0 + 4.0 * (2. * u[2] - 1.) #Period 3
                        # x[1] = 0.0 + 0.1 * (2. * u[1] - 1.) # b3
                        # x[0] = 0.0 + 0.1 * (2. * u[0] - 1.) # a3
            elif self.free_bias == True:
                if self.free_period == True:
                    x[-1] = 2.5 + 2.5 * (2. * u[-1] - 1.) #period
                    x[-2] = 1.0 + 2.5 * (2. * u[-1] - 1.) # bias
                else:
                    x[-1] = 1.0 + 2.5 * (2. * u[-1] - 1.)
            return x
        ...
        if self.amp1_mode == True:
            if self.num_bands == 1:
                band_labels = ['a1','phase1','P1','slope','mean']
            if self.num_bands == 2:
                band_labels = ['a1','phase1','P1','a2','phase2','P2','slope','mean']
            if self.num_bands == 3:
                band_labels = ['a1','phase1','P1','a2','phase2','P2','a3','phase3','P3','slope','mean']
            if self.num_bands == 4:
                band_labels = ['a1','phase1','P1','a2','phase2','P2','a3','phase3','P3','a4','phase4','P4','slope','mean']
            labels = np.append(labels,band_labels)
        elif self.num_bands > 0:
            if numspot == 0:
                if self.num_bands == 1:
                    band_labels = ['a1','b1','P1','slope','mean']
                if self.num_bands == 2:
                    band_labels = ['a1','b1','P1','a2','b2','P2','slope','mean']
                if self.num_bands == 3:
                    band_labels = ['a1','b1','P1','a2','b2','P2','a3','b3','P3','slope','mean']
                if self.num_bands == 4:
                    band_labels = ['a1','b1','P1','a2','b2','P2','a3','b3','P3','a4','b4','P4','slope','mean']
                labels = np.append(labels,band_labels)
            if numspot > 0:
                if self.num_bands == 1:
                    band_labels = ['a1','b1','P1','slope','mean']
        ...
        if self.free_bias == True:
            if self.free_period == True:
                ...
                labels = np.append(labels,['Period'])
            else:
                labels = np.append(labels,['Bias'])
...
def wave_uncertainty(inputs,length_of_observation,time_steps,color_series = False,number_of_random_samples = 1000,slope_mean = 0, slope_std = 0,bias_mean = 0, bias_std = 0):
    n_waves = len(inputs)
    n = time_steps
    m = number_of_random_samples
    x = np.linspace(0,length_of_observation,n)
    wave_gauss = np.zeros((n_waves,n,m))

    for i in range(n_waves):
        amp_gauss = np.random.normal(inputs[i,0],inputs[i,1],m)
        period_gauss = np.random.normal(inputs[i,2],inputs[i,3],m)
        phase_gauss = np.random.normal(inputs[i,4],inputs[i,5],m)
        for j in range(n):
            wave_gauss[i,j,:] = amp_gauss*np.sin(2*np.pi*x[j]/period_gauss+(phase_gauss*np.pi/180))
    
    
    if color_series == True:
        y = wave_gauss[0]+wave_gauss[1]-wave_gauss[2]-wave_gauss[3]
    elif len(inputs) == 1:
        y = wave_gauss[0]
    elif len(inputs) == 2:
        waves = wave_gauss[0]+wave_gauss[1]
        bias_gauss = np.random.normal(bias_mean,bias_std,m)
        slope_gauss = np.random.normal(slope_mean,slope_std,m)
        linear = np.zeros((n,m))
        for j in range(n):
            linear = bias_gauss+(slope_gauss*x[j])
        y = waves+linear
    elif len(inputs) == 3:
        waves = wave_gauss[0]+wave_gauss[1]+wave_gauss[2]
        bias_gauss = np.random.normal(bias_mean,bias_std,m)
        slope_gauss = np.random.normal(slope_mean,slope_std,m)
        linear = np.zeros((n,m))
        for j in range(n):
